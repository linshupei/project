layui.define(["jquery", "layer", "laytpl", "element", "form"], function(e) {
	...
	u.prototype.placeholder = function() {
		var e = this,
			r = "";
		w("input.larryms-input,.larryms-textarea,input.larry-input[type='text'],input.larry-input[type='password']").on("focus", function() {
			r = w(this).attr("placeholder");
			w(this).attr("placeholder", "")
		});
		w("input.larryms-input,.larryms-textarea,input.larry-input[type='text'],input.larry-input[type='password']").on("blur", function() {
			w(this).attr("placeholder", r)
		})
	};
	u.prototype.set = function(e) {
		var r = this;
		w.extend(true, r.config, e);
		return r
	};
	...
	u.prototype.plugin = function(e, r, t) {
		...
		if (w.isPlainObject(t)) {
			...
			this.set(t)
		} else if (!w.isPlainObject(t) && t !== undefined) {
		...
		function s() {
			return this.index = parent.layer.open({
				...
				success: function(e, r) {
					if (t == "error") {
						w("#messageBox").addClass("larry-message-error")
					} else if (t == "waring") {
						w("#messageBox").addClass("larry-message-waring")
					}
				}
			})
		}
	...
	u.prototype.open = function(e, r) {
		...
		if (r === "current") {
			...
			if (!n.shadeClose) {
				w("#layui-layer-shade" + a).on("click", function() {
					l.min(a);
					w(this).hide();
					layui.cache.layerIndex = a
				});
				var i = w("#layui-layer" + a).find(".layui-layer-max"),
					o = w("#layui-layer" + a).find(".layui-layer-min");
				o.on("click", function() {
					w("#layui-layer-shade" + a).hide();
					layui.cache.layerIndex = a
				});
				i.on("click", function() {
					w("#layui-layer-shade" + a).show()
				})
			}
			return a
		} else {
	...
	u.prototype.configFilter = function(e, r) {
		var t = {};
		for (var n in e) {
			if (w.inArray(n, r) || w.inArray(n, r) == 0) {
				t[n] = e[n]
			}
		}
		return t
	...
	u.prototype.utils = {
		fixbar: function(t) {
			...
			var l = [t.bar1, t.bar2, "larry-fanhuidingbu"],
				s = w(['<ul class="' + e + '">', t.bar1 ? '<li class="backControl" lay-type="bar1" style="' + t.bgcolor + '" ' + t.custom + "><em>返回首页</em></li>" : "", t.bar2 ? '<li class="layui-icon ' + l[1] + '" lay-type="bar2" style="' + t.bgcolor + '" ' + t.custom + "></li>" : "", '<li class="larry-icon ' + l[2] + '" id="' + r + '" lay-type="top" style="' + t.bgcolor + '"></li>', "</ul>"].join("")),
				c = s.find("#" + r),
	...
	u.prototype.panel = function() {
		w(".larry-panel .tools").off("click").on("click", function() {
			if (w(this).hasClass("larry-unfold1")) {
				w(this).addClass("larry-fold9").removeClass("larry-unfold1");
				w(this).parent(".larry-panel-header").siblings(".larry-panel-body").slideToggle()
			} else {
				w(this).addClass("larry-unfold1").removeClass("larry-fold9");
				w(this).parent(".larry-panel-header").siblings(".larry-panel-body").slideToggle()
			}
		});
		w(".larry-panel .close").off("click").on("click", function() {
			w(this).parents(".larry-panel").parent().fadeOut()
		})
	};
	...
	u.prototype.notice = function(e, r, t, n) {
		...
		var i = {
			...
			o = {
				...
				transition: { in : "pt-page-moveFromRightFade",
					out: "pt-page-fade"
				},
		...
		function g(e) {
			...
			var t = {
				href: w(e).data("url"),
				id: w(e).data("id"),
				font: w(e).children("i").data("font"),
				icon: w(e).children("i").data("icon"),
				group: r,
				title: w(e).children("cite").text(),
				addType: "page"
			};
		...
		function v(e) {
			...
			if (o.type == 1) {
				...
				if (!w(e).find(".larryms-notice-box").children(".larryms-notice").length) {
					w(e).find(".larryms-notice-box").remove()
				}
				if (!w(e).find(".larryms-notice-box").length) {
					w(e).append('<div class="larryms-notice-box notice-' + o.position + '" id="larryms-msg"></div>')
				}
				w(e).children("#larryms-msg").append(m);
				var r = "#" + w(m).attr("id");
				if (w(e).find(r).children(".larryms-msg-close").hasClass("layui-hide")) {
					setTimeout(function() {
						w(e).find(r).removeClass(o.transition. in ).addClass(o.transition.out);
						setTimeout(function() {
							w(e).find(r).remove()
						}, i.delay)
					}, o.time)
				}
				w(e).find(".larryms-notice-box .larryms-msg-close").on("click", function() {
					w(this).parents(".larryms-notice").remove();
					if (!w(e).find(".larryms-notice-box").children(".larryms-notice").length) {
						w(e).find(".larryms-notice-box").remove()
					}
				});
				w(e).find(r).on("click", function() {
					var e = w(this).children(".layui-card-body");
					v(e);
					w(this).remove();
	...
	u.prototype.noticeAllClose = function() {
		if (w(top.document).find(".larryms-notice-box").length) {
			w(top.document).find(".larryms-notice-box").remove()
		}
	};
	...
	u.prototype.player = function(e) {
		...
		w(r.audioElm).get(0).play();
		w(r.audioElm).get(0).addEventListener("ended", function() {
			w(r.audioElm).remove()
		}, false);
		return r
	};
	...
	function h(e, r, t) {
		...
		var a = w.md5("LarryMS" + e + r + "larryms.com");
		if (n == a) {
			return true
		} else {
			return false
